<template>
  ...
  <div class="image-content">
    ...
    <vuedraggable
      v-model="list.parameters.grid_items"
      style="display: flex; flex-wrap: wrap"
    >
      <template
        v-if="
          list.parameters.grid_items && list.parameters.grid_items.length > 0
        "
      >
        <el-card
          v-for="(item, dex) in list.parameters.grid_items"
          :key="dex"
          class="box-card"
          shadow="hover"
          style="
        ...
        >
          ...
          <div style="margin-top: 5px">
            <el-input v-model="item.inputTitle" placeholder="请输入标题" />
          </div>
          <el-button
            v-if="item.title == ''"
    ...
    <el-form ref="form" :model="list.parameters">
      ...
      <el-form-item>
        <div style="padding: 0 10px">
          <el-slider
            v-model="list.parameters.image_padding_lr"
            :max="50"
            :min="0"
            show-input
          />
      ...
      <el-form-item>
        <div style="padding: 0 10px">
          <el-slider
            v-model="list.parameters.image_padding_tb"
            :max="50"
            :min="0"
            show-input
          />
  ...
  export default {
    ...
    watch: {
      imgList: {
        handler(val) {
          this.tempList = []
          this.list.parameters.grid_items = val
    ...
    created() {
      this.list = this.data
      this.imgList = this.list.parameters.grid_items
    },
    ...
    methods: {
      ...
      getSelectLink(data) {
        this.list.parameters.grid_items[data.index].url = data.selectUrl
        this.list.parameters.grid_items[data.index].title = data.selectName
        this.list.parameters.grid_items[data.index].selectTitle =
          data.selectTitle
        if (data.link_type == undefined) {
          this.list.parameters.grid_items[data.index].opentype = null
        } else {
          this.list.parameters.grid_items[data.index].opentype = data.link_type
        }
      },
      getSon(data) {
        if (this.imageIndex != null) {
          this.list.parameters.grid_items[this.imageIndex].img = data[0]
