...
<script>
  import {
    ...
    addDepotPositionSaveList,
    delDepotPositionSaveList,
  } from '@/api/basic'
  export default {
    ...
    methods: {
      ...
      save() {
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (this.title === '添加') {
              const { code } = await addDepotPositionSaveList(this.formEdit)
              if (code != 200) {
                return
              }
              this.$baseMessage(
      ...
      handleDelete(row) {
        if (row.id) {
          this.$baseConfirm(
            ...
            null,
            async () => {
              const { code } = await delDepotPositionSaveList({ id: row.id })
              if (code != 200) {
                return
              }
              this.$baseMessage(
